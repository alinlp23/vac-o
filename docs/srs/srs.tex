...
\section{Requerimientos}
  ...
  \begin{enumerate}
    ...
    Adem\'as la extensi\'on debe especificar la profundidad $M$ con la que se desea realizar el ``control de calidad'' y solo aquellas secuencias que pasen el ``control de calidad'' para todas las regiones, seran consideradas para una posterior evaluaci\'on.         
  ...
  \begin{enumerate}
    ...
    A continuaci\'on se presenta el proceso de validaci\'on en pseudo-c\'odigo. Recordar que $M$ es un valor dado por la extensi\'on al momento de configuraci\'on.

    \begin{algorithmic}
      \STATE pass = $true$
      \STATE s = secuencia candidata generada por vac-o
      \FORALL{$region$ \textbf{in} $regions$}
	\STATE r = s[$region$]
	\STATE pass = QA(r, $M$)
	\IF{\textbf{not} pass}
	  \STATE \textbf{break}
    ...
    \begin{algorithm}
      \caption{QA(sequence, deep)}
      \begin{algorithmic}
	\STATE pass = $true$
	\IF{deep $\ge$ 1}	
	  \STATE mutations = generar una cola de mutaciones de ``sequence''.
	  \WHILE{\textbf{not} isEmpty(mutations) \textbf{and} pass}
	  \STATE m = first(mutations)
	  \IF{m pasa el criterio de prueba}
	    \STATE pass = QA(m, deep$-$1)
	  \ELSE
	    \STATE pass = $false$
	  \ENDIF
	  \ENDWHILE
    ...
    Basicamente, el procedimiento auxiliar \textbf{QA}, genera y recorre un arbol de mutaciones que tendra a lo sumo profundidad $M$ y diremos que una region pasa el ``control de calidad'' cuando sea posible generar el arbol completo. Es decir, cuando todos los nodos (mutaciones) del arbol, pasen el criterio de prueba especificado. Si alguna region no pasa el ``control de calidad'', entonces la secuencia se descarta.
