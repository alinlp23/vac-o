...
\section{Requerimientos}
  ...
  \begin{enumerate}
    ...
    El sistema recibe la ubicaci\'on del archivo \textit{.so} y lo carga en memoria. Si la carga es exitosa, se devuelven los parametros requeridos por la extensi\'on (\textit{[(nombre, tipo, requerido, defecto)]}) para su posterior configuraci\'on. En caso de que la carga de la extensi\'on fracase, se devuelve un mensaje de error.
    \item \textbf{Configuraci\'on de la extensi\'on:}
    El sistema recibe la lista de parametros requeridos por la extensi\'on (\textit{[(nombre, valor)]}), se realiza la validaci\'on y en caso de exito, se asignan los valores en la extensi\'on. Finalmente, se devuelve un mensaje de exito indicando que vac-o esta listo para comenzar la ejecuci\'on. En caso de algun error de validaci\'on, se devuelve un mensaje de error.
  \end{enumerate}
  ...
  \begin{enumerate}
    ...
    El sistema solicita a la extensi\'on cargada en memoria, las regiones combinatorias de interes para la optimizaci\'on. La extensi\'on debe devolver las regiones (\textit{[(inicio, fin, tipo)]}) y el sistema inicializa cada region en memoria. Para cada region, el parametro \textit{tipo} debe ser uno de los siguientes:
    \begin{itemize}
      \item Estructura secundaria (SS): Las posibles mutaciones de la region deben conservar la estructura secundaria.
      \item Codigo genetico (GC): Las posibles mutaciones de la region deben conservar las prote\'inas.      
    \end{itemize}
  ...
  \begin{enumerate}
    \item \textbf{Generar mutaci\'on de region combinatoria:}
    Para cada region combinatoria el sistema debe generar una posible mutaci\'on, que cumpla con la restricci\'on impuesta por el tipo de region y las posibles intersecciones con las demas regiones.
    \item \textbf{Generar secuencia:}
    A partir de las mutaciones de cada region combinatoria, se debe construir una nueva secuencia gen\'omica, reemplazando cada region por su mutaci\'on.
  \end{enumerate}
